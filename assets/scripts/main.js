...
const recipes = [
  ...
  "assets/recipes/shrimpscampi.json",
  "assets/recipes/garlicbread.json",
  "assets/recipes/chickpeasalad.json",
];
...
function createRecipeCards() {
  ...
  let indexMainVar = document.querySelector("main");
  if (recipes.length >= 3) {
    for (let i = 0; i < 3; i++) {
      let recipeCard = document.createElement("recipe-card");
      recipeCard.data = recipeData[recipes[i]];
      indexMainVar.appendChild(recipeCard);
    }
  }
}
...
function bindShowMore() {
  ...
  showMoreBtn.addEventListener("click", showMoreFunc);

  function showMoreFunc() {
    if (showMoreBtn.textContent == "Show less") {
      while (mainVar.firstChild) {
        mainVar.removeChild(mainVar.firstChild);
      }
      createRecipeCards();
